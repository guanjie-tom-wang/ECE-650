...
class StreetOperation:
        ...
        if street is not None:
            ...
            for st in self.names:
                if st == street[0]:
                    flag = True
            if flag:
                print("Error: the street has already exist", file=sys.stderr)
            else:
    ...
    def generate(self):
        ...
        for edge in range(len(edgeList) - 1):
            ...
            print(str('<') + str(a) + str(',') + str(b) + str('>') + str(','), file=sys.stdout)

        if (len(edgeList) >= 1):
            print(str('<') + str(verticesLookUp[edgeList[(len(edgeList) - 1)].src]) + str(',') + str(
                verticesLookUp[edgeList[(len(edgeList) - 1)].dst]) + str('>'), file=sys.stdout)
        print('}', file=sys.stdout)
...
def main():
    ...
    while True:
        ...
        if len(c) == 0:
            ...
            continue
        if c[0] in ("add", "gg ", "mod ", "rm ", "rm", "mod", "gg"):
